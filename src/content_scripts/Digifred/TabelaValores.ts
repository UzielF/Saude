...
interface TabelaValores
{
    Itens: Item[];
    CarregarItens: () => void;
    ObterValorProfissional: (item: Item) => void;
    ObterValorClinica: (item: Item, res: string) => void;
    ExtrairValorClinica: (item: Item, nomeClinica: string, res: string) => void;
    ObterValorGrupo: (item: Item) => void;
    ExtrairValorGrupo: (item: Item, res: string) => void;
    ObterValorTotal: () => void;
    ExtrairValorTotal: (res: string) => void;
    CriarVisualizacao: () => void;
    EnviarRequest: (url: string, payload: string, resultHandler: (res: string) => void) => void;
    ObterDatasFaturamentoAtual: () => [string, string];
}

const TABELA_VALORES: TabelaValores =
{
    Itens: [],
    ...
    ObterValorGrupo(item: Item): void
    {
        ...
        TABELA_VALORES.EnviarRequest(url, payload, (res: string) => TABELA_VALORES.ExtrairValorGrupo(item, res));
    },
    ExtrairValorGrupo(item: Item, res: string): void
    {
    ...
    ObterValorProfissional: async function (item: Item)
    {
        ...
        TABELA_VALORES.EnviarRequest(url, payload, (res: string) => TABELA_VALORES.ObterValorClinica(item, res));
    },
    ObterValorClinica: async function (item: Item, res: string)
    {
        ...
        x.onreadystatechange = () =>
        {
            ...
            {
                if (clinicas[i]!.children[0]!.textContent !== item.Clinica) continue;
    ...
    ExtrairValorClinica(item: Item, nomeClinica: string, res: string): void
    {
        ...
        let valores: { [key: string]: string; } = {};

        let clinicaAtual = "";
        for (let i = 1; i < tabela.length - 1; i++)
        {
            if (tabela[i]!.children.length === 1) clinicaAtual = tabela[i]!.children[0]!.textContent!.trim();

            if (tabela[i]!.children.length === 2) valores[clinicaAtual] = tabela[i]!.children[1]!.textContent!;
        }
    ...
    ObterValorTotal: async function ()
    {
        ...
        TABELA_VALORES.EnviarRequest(url, payload, (res: string) => TABELA_VALORES.ExtrairValorTotal(res));
    },
    ExtrairValorTotal(res: string): void
    {
        let parser = new DOMParser();
    ...
    CriarVisualizacao(): void
    {
        ...
        let imagemEsquerda = <HTMLImageElement>DOM.CriarElemento("img", "img-botaoToggle-esquerda");
        imagemEsquerda.src = `chrome-extension://${chrome.i18n.getMessage("@@extension_id")}/assets/Images/Icons/Esquerda.png`;
        let imagemDireita = <HTMLImageElement>DOM.CriarElemento("img", "img-botaoToggle-direita");
        ...
        for (let item of TABELA_VALORES.Itens)
        {
            ...
            if (item.Codigo !== "")
            {
                if (item.Tipo === TipoDeEntrada.Profissional) TABELA_VALORES.ObterValorProfissional(item);
                else TABELA_VALORES.ObterValorGrupo(item);
            }
            else TABELA_VALORES.ObterValorTotal();
    ...
    EnviarRequest: async function (url: string, payload: string, resultHandler: (res: string) => void)
    {
        let req = new XMLHttpRequest();
        req.open("POST", url, true);
        req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        req.onreadystatechange = () => { if (req.readyState === 4) resultHandler(req.response); };
        req.send(payload);
    },
    ObterDatasFaturamentoAtual(): [string, string]
    {
