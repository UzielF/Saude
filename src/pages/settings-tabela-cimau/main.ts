...
interface ConfNomeComputador {
    Nome: string;
    Salvar(): void;
    Carregar(): void;
}

const CONF_NOME_COMPUTADOR: ConfNomeComputador =
{
    Nome: "",
    Salvar(): void {
        let input = <HTMLInputElement>DOMH.ObterPelaClasse("input-nomeComputador");
    ...
    Carregar(): void {
        ...
        chrome.storage.local.get("NOME_COMPUTADOR").then(r => {
            if (!r.hasOwnProperty("NOME_COMPUTADOR")) return;
