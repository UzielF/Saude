function EstaNoPainelDeBeneficios() {
    ...
    if (botaoAvancarOuGravar.textContent === "Gravar")
        return true;
    return false;
}
...
function AdicionarEventHandlerAoBotaoAvancar() {
    ...
    if (botaoAvancar.textContent !== "Avançar")
        return;
...
function AdicionarEventHandlerAoBotaoVoltar() {
    if (!EstaNoPainelDeBeneficios())
        return;
    let secaoConcessaoDeBeneficios = document.getElementsByTagName("table")[3];
    if (secaoConcessaoDeBeneficios === undefined)
...
function CriarCampoAutorizacaoCIMAU() {
...
function ObterValorTotal() {
    ...
    if (listaBeneficios[1].textContent === "Nenhum adicionado..")
        return "R$ 0,00";
...
function IniciarLoop() {
    if (!EstaNoPainelDeBeneficios() || LoopAtualizacaoValorTotal !== null)
        return;
    LoopAtualizacaoValorTotal = setInterval(AtualizarValorTotal, 450);
}
function TerminarLoop() {
    if (EstaNoPainelDeBeneficios() || LoopAtualizacaoValorTotal === null)
        return;
